...
def _setup_axes(tr: LoadedTrace, figsize, dpi):
    n_series = len(tr.series)
    layout = single_layout_spec(max(n_series, 1))
    fig = plt.figure(figsize=figsize, dpi=dpi)
    if n_series == 0:
        # Every optional panel was dropped for lack of a channel. That is an
        # allowed preset outcome, so the BEV takes the width the series column
        # would have used rather than the renderer failing on an empty layout.
        left, bottom, _, height = layout["bev"]
        right = layout["series"][0][0] + layout["series"][0][2]
        ax_bev = fig.add_axes((left, bottom, right - left, height))
        ax_series = []
    else:
...
def _draw_static(tr: LoadedTrace, ax_bev, ax_series, ax_instruments=None,
    ...
    :param ax_instruments: T6 gauge axes; taken from the figure when omitted.
