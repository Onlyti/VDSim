...
class CheckCurrencyTest(unittest.TestCase):
    ...
    def test_pending_with_a_reason_passes(self):
        """An integration merge voids the old count; PENDING is the honest state."""
        doc = make_doc(tests="PENDING", presets="PENDING", commit="PENDING",
                       date="PENDING", toolchain="PENDING",
                       extra="pending:  tree replaced by an integration merge")
        self.assertEqual(self.run_gate(doc, measured=(9, 9)), [])

    def test_pending_without_a_reason_fails(self):
        """PENDING must not become a silent way to publish no number at all."""
        doc = make_doc(tests="PENDING", presets="PENDING", commit="PENDING",
                       date="PENDING", toolchain="PENDING")
        problems = self.run_gate(doc, measured=(9, 9))
        self.assertTrue(any("pending:" in p for p in problems), problems)

    def test_pending_does_not_disable_the_count_drift_check(self):
        """A real number is still compared; only the placeholder short-circuits."""
...
class RealDocumentTest(unittest.TestCase):
    ...
    def test_repo_document_has_currency_block(self):
        ...
        if str(block["tests"]).strip() == cvc.PENDING_TOKEN:
            self.assertTrue(block.get("pending"),
                            "a PENDING count must record why it is pending")
        else:
