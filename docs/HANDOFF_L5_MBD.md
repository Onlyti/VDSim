...
0. **[최우선 — VLA thesis plant = 첫 고객 = 첫 BETA TESTER. 브랜치 `VDSim-Thesis`]**
   ...
   **다음 세션 필수 = Cursor 산출물 검증·리뷰·폴리시 게이트** (Claude 책임): `git diff` 리뷰 +
   ...
   AXLE값→per-wheel B=Ca/2 주의) · `VDSimPlant` 래퍼(vdsim_lab 위) · smoke 3종(#2=patch
   brake+turn combined grip-loss). 이미 있음: SimSession lockstep · MF96 combined ·
   `tire_forces_wheel()`(마찰원용 contact-frame) · per-wheel GT. 함정: wheel_spin=vx/R seed,
   plant obs=true_state, 차량 kinematic-blend 우회/VLOW.
...
- Follow-up DONE (2026-06-17): gyro spin-axis STEER tilt (a steered spinning wheel's momentum
  tilts about body-z; camber tilt omitted). KC compliance ATTEMPTED then REVERTED: the lumped
  compliance_targets_rad + default bushings gave ~1.5 deg/g compliance steer (~5-10x too big,
  unvalidated) — it needs the Adams/KC reference (Tier 1) to set bushing rates before shipping.
  Spin-up reaction reclassified as a drivetrain-mount effect (not unsprung). 378/378 green.
- L5 high-fidelity dynamics feature-complete: energy-consistent general-surface free-3D
  ...
  progressive damper + gyroscopic wheel-spin (steer-tilted). Remaining: KC compliance (blocked
  on Tier-1 Adams validation), gyro camber tilt.
