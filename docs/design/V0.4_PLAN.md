...
Status: planning (2026-06-06). **All stunt capability ships in v0.4.0** — no v0.5/v0.6
split. v0.3.0 remains drivetrain + LuGre only (`V0.2_DRIVETRAIN.md`, `V0.2_TIRE_LUGRE.md`).
...
| Suite | Scope |
|-------|--------|
| ctest 187 | **unchanged** on default L2/L3 flat (no stunt in default path) |
...
| # | Milestone | WS | Blocker for |
|---|-----------|-----|-------------|
| M1 | T23 C++ jump + contact wired | A | all stunt |
| M2 | RampGround + `jump_ramp.yaml` + GUI preset | A,B,D | curved track |
| M3 | CurvedGround + banked scenario | B | Ld5 on curves |
| M4 | Ld5 core + jump on Ld5 | C | loop |
| M5 | MeshGround loop + `vertical_loop.yaml` | B,C | ship |
| M6 | Loop validation suite + theory doc | E | tag v0.4.0 |

Tag **v0.4.0** only when M1–M6 are green.
