...
| Full suite: **PENDING** (`validation` preset — see the currency block below) | — |
...
```text
tests:    PENDING
pending:  cosim/CMakeLists.txt registers the new rl_env target, so this
pending:  tree registers one test more than main; the canonical count is
pending:  whatever the CI validation job measures three times in a row on
pending:  the Q23 PR (local runs are candidates, not canonical).
...
```sh
cmake --build build -j && (cd build && ctest --output-on-failure)   # count in the currency block
