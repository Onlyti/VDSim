...
#: Placeholder written in place of a measurement while the tree that the
#: last number belonged to no longer exists (integration merge). It is a
#: state, not a value: publishing the old number would be a stale claim and
#: inventing a new one without a canonical run would be a fabricated claim.
...
def check_currency(block, registered, measured, commit, presets_sha,
    ...
    if str(block["tests"]).strip() == PENDING_TOKEN:
        # Pre-measurement state. There is no number to compare against, so the
        # gate only enforces that the placeholder is deliberate and says why.
        # `hardcoded_test_counts` still runs in main(), so PENDING here cannot
        # be used to smuggle a count into the surrounding prose.
        if not str(block.get("pending", "")).strip():
            return ["tests: %s requires a 'pending:' line stating why no "
                    "canonical measurement exists yet" % PENDING_TOKEN]
        return []
